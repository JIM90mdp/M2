...
// Tu código acá:
document.querySelector('#createdBy').innerHTML += " Juan I. Mascarenhas"
...
function buildToDo(todo, index) { // <div> id="" className="" <span> todo.description <span> <div>
  // Tu código acá:
  const toDoShell = document.createElement('div');
  toDoShell.className = 'toDoShell';
  //toDoShell.classList.add('toDoShell'); //tambien se puede asignar/definir la clase así
  //toDoShell.setAttribute('class', 'toDoShell'); //tambien se puede definir la clase así
  ...
  toDoText.className = todo.complete ? 'completeText' : ''; // forma de hacerlo lo que está arriba en una linea.
  toDoShell.appendChild(toDoText); // agrega un nodo (hijo) a la clase
...
function buildToDos(toDos) {
  // Tu código acá:
//return toDos.map((todo, i) => {return buildToDo(todo, i)});
  return toDos.map(buildToDo); // forma de hacerlo en una linea
//  const buildMap = toDos.map(function(todo, index){
...
//  4) Iterar sobre el resultado devuelto por la función 'buildToDos' e ir agregndo cada elemento como hijo de 'toDoContainer'
...
function displayToDos() { //hasta acá <---
  // Tu código acá:
...
// Tu código acá:
document.getElementById('addButton').addEventListener('click', addToDo) //la funsión se guarda y se ejecuta cuando entra en el listener, de lo contrario la función con los parentesis ejecutaría la funcion en el momento que javascript lee la linea de codigo.
  // const addButton = document.querySelector('#addButton');
  // addButton.onclick = function() { addToDo() } // hace lo mismo (en parte) con addeventlinestener podes crear 15 funciones que hagan en paralelo algo que quiras que hagas. pero con onclick en vez de dejarme agregar muchas, solo me deja poner una. y si yo hago addButon.onclick de nuevo, voy a estar pisando la función onclick de la variable addButon. Solo sirve para el evento 'click', el resto de los eventos se hace con el metodo addEvento.
